...
namespace default {
  CDefaultScanner::CDefaultScanner(
              ...
              if (p_data_handler_->FileContentNeed()) {
                ...
                if (fd != INVALID_HANDLE_VALUE) {
                  ...
                  while (ReadFile(fd, content, kBlockSize, &dwLen, NULL)) {
                    b32 res = p_data_handler_->OnFileContent(filepath, content,
                                                             dwLen);
                    if (res <= 0) {
                      break;
                    }
            ...
            for (; (b8 *)file_info < query_buffer.get() + query_buffer_size;
              ...
              if (file_info->FileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                ...
                if (file_size == 0) {
                  ...
                  if (p_data_handler_->FileContentNeed()) {
                    scope::ScopedHandle fd(CreateFile(
                    ...
                    if (fd != INVALID_HANDLE_VALUE) {
                      ...
                      ulong dwLen = 0;
                      while (ReadFile(fd, content, kBlockSize, &dwLen, NULL)) {
                        b32 res = p_data_handler_->OnFileContent(
                            filepath, content, dwLen);
                        if (res <= 0) {
                          break;
                        }
                      }
    ...
    while (!vec_dir.empty()) {
      ...
      if (p_dir != NULL) {
        while (NULL != (p_file_info = readdir(p_dir))) {
          ...
          if (p_file_info->d_type == DT_FIFO ||
              p_file_info->d_type == DT_SOCK || p_file_info->d_type == DT_LNK ||
              p_file_info->d_type == DT_CHR || p_file_info->d_type == DT_BLK)
            goto NEXT;
